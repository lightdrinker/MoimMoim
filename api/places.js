...
export default async function handler(req, res) {
  ...
  // ── 카테고리별 메뉴 사전 (1글자 모호 단어는 제거 — '전','회','진','럼' 등은 substring 노이즈가 너무 큼)
  // 각 메뉴는 합성형/명확한 명사만. 짧고 흔한 한국어 단어와 충돌하지 않게.
  const MENU_DICT = {
    korean: [
      '삼겹살','목살','오겹살','갈비살','차돌박이','우삼겹','LA갈비','갈비탕','갈비찜','갈비','등심','안심','육회',
      '김치찌개','된장찌개','부대찌개','순두부찌개','순두부','김치찜','순대국','순댓국','감자탕','뼈해장국','해장국',
      '설렁탕','곰탕','육개장','삼계탕','닭한마리','닭볶음탕','곱창','대창','막창','닭갈비','제육볶음','제육','불고기',
      '떡갈비','족발','보쌈','수육','김치말이','국밥','비빔밥','회덮밥','연어덮밥','참치덮밥','오징어덮밥',
      '떡볶이','순대','김밥','라면','만두','칼국수','콩국수','잔치국수','수제비','냉면','막국수','국수',
      '김치전','파전','부추전','해물파전','모듬전','메밀전병','전골','뚝배기','한정식','쌈밥',
    ],
    chinese: [
      '짜장면','짬뽕','탕수육','마라탕','마라샹궈','꿔바로우','양꼬치','마파두부','짜장','쟁반짜장','삼선짜장',
      '볶음밥','동파육','깐풍기','유린기','딤섬',
    ],
    japanese: [
      '초밥','스시','사시미','우니','이자카야','야키토리',
      '라멘','돈코츠','쇼유','텐푸라','텐동','오므라이스','돈까스','규동','오니기리','가츠동','우니덮밥',
    ],
    western: [
      '파스타','피자','스테이크','리조또','라자냐','뇨끼','샐러드','브런치','수프','감자튀김',
      '햄버거','부리또','타코','퀘사디아','샌드위치','스파게티','크림파스타','로제파스타','토마토파스타',
      '봉골레','알리오올리오','까르보나라','오일파스타',
    ],
    cafe: [
      '와플','팬케이크','케이크','컵케이크','마카롱','마들렌','쿠키','크로와상','크로플','크로크무슈','베이글',
      '도넛','식빵','크림빵','앙버터','휘낭시에','스콘','파운드케이크','치즈케이크','티라미수','푸딩','타르트','파이',
      '커피','아메리카노','라떼','카푸치노','에스프레소','드립커피','콜드브루','말차','녹차','홍차','얼그레이',
      '스무디','에이드','주스','빙수','젤라또','아이스크림','밀크티','버블티',
    ],
    bar: [
      '맥주','생맥주','수제맥주','크래프트맥주','크래프트','IPA','라거','흑맥주','스타우트','에일','밀맥주','필스너',
      '와인','레드와인','화이트와인','로제와인','스파클링','샴페인',
      '사케','정종','청주','막걸리','동동주','전통주','소주','과실주','매실주','복분자','칵테일','하이볼','진토닉',
      '모히토','마티니','데킬라','위스키','보드카',
      '치킨','후라이드','양념치킨','간장치킨','파닭','순살','뼈닭','반반','닭발','닭똥집','노가리','쥐포',
      '골뱅이','골뱅이무침','쭈꾸미','낙지','족발','보쌈','소시지','나초','피쉬앤칩스','콘치즈','두부김치','감자전','옥수수치즈',
    ],
  };
  ...
  // cuisine을 받아 그 카테고리 사전만 사용. cuisine 모르면 빈 배열 (틀린 카드 방지).
  const extractMenus = (snippetText, keywordRaw, cuisine) => {
    if (!cuisine || !MENU_DICT[cuisine]) return [];
    const dict = MENU_DICT[cuisine];
    const sorted = [...new Set(dict)].sort((a, b) => b.length - a.length);
    ...
    if (keywordRaw) {
      keywordRaw.split(/\s+/).forEach(w => {
        if (dict.includes(w) && !counts.has(w)) counts.set(w, 1);
      });
    }
  ...
  try {
    if (action === 'nearby') {
      ...
      enriched.forEach(p => {
        ...
        p.menus = extractMenus(`${menuText} ${blogText}`, keyword, p.cuisine);
